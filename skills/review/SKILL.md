...
3. Run the `adr-distiller` subcommand:
   ...
   ```
   Read `skills/adr-distiller/SKILL.md` and execute its procedure. The distiller uses the `adrs` field to locate ADRs, compares mtimes against the cache, and refreshes `adr-distilled.md`. On first run (no cache), it creates the file and cache from scratch.
