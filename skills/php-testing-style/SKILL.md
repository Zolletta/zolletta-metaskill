...
> **Scope boundary**: the `/zolletta-metaskill patterns` subcommand runs `test_structure_scanner.py` which produces a "Missing tests" table — a **structural** check (no `*Test.php` file exists). That is a structural finding owned by `patterns`. This skill owns **coverage gap analysis** using PHPUnit's coverage report — checking whether code is actually exercised by tests, regardless of file naming. Do not duplicate the structural check. If `test_structure_scanner.py` already flagged a file as structurally missing a test, reference that finding but focus on whether the code is actually covered.
...
Do not duplicate the structural "missing test file" check from `test_structure_scanner.py` (owned by `patterns`). This skill owns coverage analysis: whether code is actually exercised by tests, not whether a `*Test.php` file exists.
