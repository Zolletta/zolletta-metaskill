...
|------------|------|-------|
| `setup` | `setup/SKILL.md` | Project initialization — creates `.zolletta-metaskill/settings.json`, detects language, tests tokensave/graphify availability |
...
1. If it **exists**, read it and proceed to the requested subcommand. The subcommand may read `language`, `tokensave_available`, `graphify_available`, `python_code_style_available`, `python_testing_patterns_available`, `external_review_model`, and `reports_dir` from it.
...
When any subcommand calls a tokensave or graphify MCP tool and receives a **tool-not-found** or **server-not-found** error (the MCP server is not registered or not responding), or when `skill invoke` returns "Skill not found" for a Python review skill:

1. **Update `settings.json`**: set the corresponding flag to `false`:
   - `tokensave_*` failure → `tokensave_available: false`
   - `graphify_*` failure → `graphify_available: false`
...
This handler applies to every subcommand that uses tokensave or graphify (`patterns`, `documentor`, `external-review`, `review`) and to `review` when it launches Python skill subagents. Each subcommand's SKILL.md links back to this section.
