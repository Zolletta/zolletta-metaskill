...
How to use code graph tools (tokensave, GitNexus, Graphify) during design pattern analysis.

> This file narrows down any eventual general rule about code exploration, i.e. [code-exploration-rules.md](~/.agents/rules/code-exploration-rules.md), [gitnexus-rules.md](~/.agents/rules/gitnexus-rules.md), [tokensave-rules.md](~/.agents/rules/tokensave-rules.md). All files in `~/.agents/rules/` are the single source of truth for their domain.
...
2. For each top candidate, use `tokensave_context` or `gitnexus context` to understand its responsibilities **without reading the full file**
3. Use `tokensave_callees` or `gitnexus context` to list what the class calls — group calls by domain (SQL, HTTP, formatting, etc.)
4. Apply the "reason to change" test based on the domain grouping
5. **Before splitting**: run `gitnexus impact` (or `tokensave_callers`) to assess blast radius — warn the user if risk is HIGH or CRITICAL
6. After splitting: run `tokensave_affected` to find which tests to run

## Patterns-specific narrowing

- **Before splitting a God class**: run `gitnexus impact` (or `tokensave_impact`) to assess blast radius. Warn the user if risk is HIGH or CRITICAL. Narrows "before editing any symbol" → "before splitting a class".
...
- **Parallel test file analysis**: after `scan_test_classes.py --show-methods` identifies test God classes, spawn one subagent per class to list which SUTs it tests and whether each SUT has its own source file.
