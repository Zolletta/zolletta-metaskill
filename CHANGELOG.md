...
- **Tool-failure handler** — when any subcommand calls a tokensave MCP tool and receives a tool-not-found / server-not-found error, or when `skill invoke` returns "Skill not found" for a Python review skill, it updates the corresponding `*_available` flag in `settings.json` to `false`, prints the "not installed" message explaining why zolletta benefits from the tool/skill, and continues with grep/read fallback or skips the missing skill's review area
- **`reference/tool-messages.md`** — shared "not installed" messages for tokensave, python-code-style, and python-testing-patterns, explaining why zolletta benefits from each tool/skill with homepage links (where applicable), used by both setup and the tool-failure handler
