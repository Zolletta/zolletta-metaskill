"""Tests for ``dependency_inversion_scanner`` — PHP DIP validator.
...
class TestIsFactory:
    ...
    def test_builder_in_name(self) -> None:
        assert DependencyInversionScanner._is_factory("QueryBuilder") is True

    def test_is_factory_no_match_returns_false(self) -> None:
        assert DependencyInversionScanner._is_factory("UserService") is False
...
class TestIsRealDependency:
    ...
    def test_built_in_stdClass(self) -> None:
        assert DependencyInversionScanner._is_real_dependency("stdClass") is False

    def test_built_in_DateTime(self) -> None:
        assert DependencyInversionScanner._is_real_dependency("DateTime") is False

    def test_built_in_Exception(self) -> None:
        assert DependencyInversionScanner._is_real_dependency("Exception") is False
...
class TestScanModule:
    ...
    def test_returns_finding_objects(self, tmp_path: Path) -> None:
        ...
        findings = DependencyInversionScanner.scan_module(module)
        assert len(findings) == 1
        assert isinstance(findings[0], Finding)
