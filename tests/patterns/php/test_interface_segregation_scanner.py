"""Tests for ``interface_segregation_scanner`` — PHP ISP validator.
...
from zolletta_metaskill.patterns.php.interface_segregation_scanner import (
...
class TestIsInterface:
    ...
    def test_abstract_no_attributes_is_interface(self) -> None:
        ...
        assert InterfaceSegregationScanner._is_interface(cls) is True
...
class TestFindImplementers:
    ...
    def test_find_implementers_finds_implementer_returns_single_item(self) -> None:
        ...
        result = InterfaceSegregationScanner._find_implementers("IRepo", [interface, impl])
        assert result == [impl]

    def test_find_implementers_no_implementers_returns_empty_list(self) -> None:
        interface = ClassInfo(name="IRepo", lineno=1, end_lineno=5)
        other = ClassInfo(name="Foo", lineno=6, end_lineno=10)
        result = InterfaceSegregationScanner._find_implementers("IRepo", [interface, other])
        assert result == []

    def test_find_implementers_multiple_implementers_returns_multiple_items(self) -> None:
        interface = ClassInfo(name="IRepo", lineno=1, end_lineno=5)
        impl_a = ClassInfo(name="UserRepo", lineno=6, end_lineno=10, bases=["IRepo"])
        impl_b = ClassInfo(name="OrderRepo", lineno=11, end_lineno=15, bases=["IRepo"])
        unrelated = ClassInfo(name="Foo", lineno=16, end_lineno=20)
        result = InterfaceSegregationScanner._find_implementers(
            "IRepo", [interface, impl_a, impl_b, unrelated]
        )
...
class TestScanModule:
    ...
    def test_fat_interface_flagged(self, tmp_path: Path) -> None:
        ...
        methods = [_make_method(f"method_{i}", lineno=i + 1) for i in range(8)]
        cls = ClassInfo(
            name="FatInterface",
            lineno=1,
            end_lineno=20,
    ...
    def test_interface_at_threshold_not_flagged(self, tmp_path: Path) -> None:
        ...
        methods = [_make_method(f"method_{i}", lineno=i + 1) for i in range(7)]
        cls = ClassInfo(
    ...
    def test_interface_below_threshold_not_flagged(self, tmp_path: Path) -> None:
        ...
        methods = [_make_method(f"method_{i}", lineno=i + 1) for i in range(3)]
        cls = ClassInfo(
            name="SmallInterface",
            lineno=1,
            end_lineno=10,
    ...
    def test_custom_min_methods(self, tmp_path: Path) -> None:
        ...
        methods = [_make_method(f"m_{i}", lineno=i + 1) for i in range(4)]
        cls = ClassInfo(
            name="Smallish",
            lineno=1,
            end_lineno=10,
        ...
        assert len(InterfaceSegregationScanner.scan_module(module, min_methods=3)) == 1
    ...
    def test_multiple_fat_interfaces(self, tmp_path: Path) -> None:
        ...
        cls_a = ClassInfo(name="FatA", lineno=1, end_lineno=20, methods=methods_a, is_abstract=True)
        cls_b = ClassInfo(
    ...
    def test_syntax_error_returns_empty(self, tmp_path: Path) -> None:
        ...
        assert InterfaceSegregationScanner.scan_module(module) == []
    ...
    def test_moduleinfo_empty_classes_returns_empty_list(self, tmp_path: Path) -> None:
    ...
    def test_finding_file_path(self, tmp_path: Path) -> None:
        ...
        methods = [_make_method(f"m_{i}", lineno=i + 1) for i in range(8)]
        cls = ClassInfo(name="Fat", lineno=1, end_lineno=20, methods=methods, is_abstract=True)
        module = ModuleInfo(
            path=tmp_path / "Fat.php",
            language="php",
            classes=[cls],
    ...
    def test_finding_line_number(self, tmp_path: Path) -> None:
        ...
        methods = [_make_method(f"m_{i}", lineno=i + 5) for i in range(8)]
        cls = ClassInfo(name="Fat", lineno=5, end_lineno=30, methods=methods, is_abstract=True)
        module = ModuleInfo(
            path=tmp_path / "Fat.php",
            language="php",
            classes=[cls],
...
class TestScanFile:
    ...
    def test_fat_interface_detected(self, tmp_path: Path) -> None:
        ...
        _write_php(
            ...
            + "".join(f"    public function method_{i}();\n" for i in range(8))
            + "}\n",
        )
    ...
    def test_write_php_custom_threshold_contains_3_methods(self, tmp_path: Path) -> None:
        ...
        findings = InterfaceSegregationScanner.scan_file(f, min_methods=2)
        assert len(findings) == 1
    ...
    def test_multiple_fat_interfaces(self, tmp_path: Path) -> None:
        ...
        _write_php(
            ...
            "<?php\n"
            "interface FatA {\n"
            + "".join(f"    public function a{i}();\n" for i in range(8))
            + "}\n"
            "interface FatB {\n"
            + "".join(f"    public function b{i}();\n" for i in range(9))
            + "}\n",
        )
        findings = InterfaceSegregationScanner.scan_file(f)
        assert len(findings) == 2
