...
from zolletta_metaskill.adr.adr_orchestrator import main
...
class TestMain:
    ...
    def test_main_with_adrs(
        ...
        monkeypatch.setattr(sys, "argv", [
            "prog", "--docs-dir", str(docs), "--adrs-path", "adr",
    ...
    def test_main_empty_adrs_path(
        ...
        cache_dir = tmp_path / ".zolletta-metaskill"
        monkeypatch.setattr(sys, "argv", [
            "prog", "--docs-dir", str(docs), "--adrs-path", "",
            "--cache-dir", str(cache_dir), "--json",
        ])
        rc = main()
        out = capsys.readouterr().out
    ...
    def test_main_nonexistent_docs_dir(
        ...
        cache_dir = tmp_path / ".zolletta-metaskill"
        monkeypatch.setattr(sys, "argv", [
            "prog", "--docs-dir", str(tmp_path / "nope"),
            "--adrs-path", "adr", "--cache-dir", str(cache_dir),
        ])
        rc = main()
    ...
    def test_main_nonexistent_docs_dir_json(
        ...
        monkeypatch.setattr(sys, "argv", [
            "prog", "--docs-dir", str(tmp_path / "nope"),
            "--adrs-path", "adr", "--cache-dir", str(cache_dir), "--json",
        ])
        rc = main()
        out = capsys.readouterr().out
        assert rc == 1
