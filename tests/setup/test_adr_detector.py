...
from zolletta_metaskill.setup.adr_detector import ADRDetector
...
def _write_adr(
    ...
    title: str,
    status: str = "Accepted",
    decision: str = "We decided to do X.",
) -> None:
...
class TestDetectAdrs:
    ...
    def test_adrs_in_subdir(self, tmp_path: Path) -> None:
        ...
        assert ADRDetector.detect_adrs(docs) == "adr"
    ...
    def test_adrs_in_decisions_subdir(self, tmp_path: Path) -> None:
        ...
        assert ADRDetector.detect_adrs(docs) == "decisions"
    ...
    def test_adrs_scattered_in_docs_root(self, tmp_path: Path) -> None:
        ...
        assert ADRDetector.detect_adrs(docs) == ""
    ...
    def test_no_docs_dir_returns_none(self, tmp_path: Path) -> None:
        assert ADRDetector.detect_adrs(tmp_path / "nonexistent") is None

    def test_grep_false_positive_filtered(self, tmp_path: Path) -> None:
        """Files mentioning 'adr' but no ADR headings → None."""
        docs = tmp_path / "docs"
        docs.mkdir()
        (docs / "notes.md").write_text("We should write ADRs someday.\n", encoding="utf-8")
        assert ADRDetector.detect_adrs(docs) is None
    ...
    def test_grep_unavailable_fallback_to_rglob(self, tmp_path: Path) -> None:
        ...
        with patch(
            "zolletta_metaskill.setup.adr_detector.ADRDetector._grep_adr_files",
            return_value=None,
        ):
            assert ADRDetector.detect_adrs(docs) == "adr"
...
class TestGrepAdrFiles:
    ...
    def test_grep_finds_adr_mention(self, tmp_path: Path) -> None:
        ...
        result = ADRDetector._grep_adr_files(docs)
    ...
    def test_grep_returns_none_when_grep_missing(self, tmp_path: Path) -> None:
        docs = tmp_path / "docs"
        docs.mkdir()
        with patch("zolletta_metaskill.setup.adr_detector.shutil.which", return_value=None):
            assert ADRDetector._grep_adr_files(docs) is None

    def test_grep_returns_none_on_timeout(self, tmp_path: Path) -> None:
        docs = tmp_path / "docs"
        docs.mkdir()
        with patch(
            "zolletta_metaskill.setup.adr_detector.subprocess.run",
...
class TestScanAdrHeadings:
    ...
    def test_finds_adr_heading(self, tmp_path: Path) -> None:
        ...
        result = ADRDetector._scan_adr_headings([f])
        assert len(result) == 1
        assert result[0] == f
    ...
    def test_scan_adr_headings_multiple_formats_returns_3(self, tmp_path: Path) -> None:
        f1 = tmp_path / "a.md"
...
class TestDetermineAdrsPath:
    ...
    def test_determine_adrs_path_single_subdir_returns_adr(self, tmp_path: Path) -> None:
        ...
        assert ADRDetector._determine_adrs_path([f1, f2], docs) == "adr"

    def test_determine_adrs_path_docs_root_returns_empty(self, tmp_path: Path) -> None:
        docs = tmp_path / "docs"
        docs.mkdir()
        f1 = docs / "0001.md"
        f1.write_text("# ADR-001\n", encoding="utf-8")
        assert ADRDetector._determine_adrs_path([f1], docs) == ""

    def test_determine_adrs_path_multiple_subdirs_returns_empty(self, tmp_path: Path) -> None:
        docs = tmp_path / "docs"
        docs.mkdir()
