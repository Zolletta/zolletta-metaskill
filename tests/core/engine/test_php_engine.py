...
def test_phpengine_language_property_returns_php() -> None:
    assert PHPEngine().language == "php"


def test_file_extensions_file_extensions_returns_single_item() -> None:
    assert PHPEngine().file_extensions() == [".php"]


def test_test_file_glob_returns_pattern() -> None:
...
@pytest.mark.skipif(not TS_PHP_AVAILABLE, reason="tree-sitter-php not installed")
def test_skipif_parse_interface_returns_2(tmp_path: Path) -> None:
    engine = PHPEngine()
    src = """<?php
...
@pytest.mark.skipif(not TS_PHP_AVAILABLE, reason="tree-sitter-php not installed")
def test_skipif_parse_trait_returns_1(tmp_path: Path) -> None:
    engine = PHPEngine()
    src = """<?php
trait Loggable {
...
@pytest.mark.skipif(not TS_PHP_AVAILABLE, reason="tree-sitter-php not installed")
def test_skipif_parse_throws_returns_empty_list(tmp_path: Path) -> None:
    engine = PHPEngine()
    src = """<?php
class Worker {
...
@pytest.mark.skipif(not TS_PHP_AVAILABLE, reason="tree-sitter-php not installed")
def test_skipif_parse_lineno_returns_4(tmp_path: Path) -> None:
    engine = PHPEngine()
    src = """<?php
