...
Implements the :class:`~zolletta_metaskill.core.engine.language_engine.LanguageEngine`
...
:class:`~zolletta_metaskill.core.structs.ModuleInfo`.
    ...
    def _build_imports(self, node: Node, source: bytes) -> list[ImportInfo]:
        ...
        if group_node is not None:
            for group_clause in self._children_by_type(group_node, "namespace_use_clause"):
                info = self._import_from_clause(group_clause, source, lineno, prefix=prefix)
                if info is not None:
                    imports.append(info)
