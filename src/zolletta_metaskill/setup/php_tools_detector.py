...
Usage:
    python3 php_tools_detector.py [directory]
...
class PHPToolsDetector:
    ...
    def detect_php_tools_from_composer(composer_path: Path) -> dict[str, bool]:
        ...
        if not composer_path.exists():
            return {tool: False for tool, _, _ in PHPToolsDetector._TOOLS}

        content = composer_path.read_text(encoding="utf-8")
        for tool, package, _ in PHPToolsDetector._TOOLS:
    ...
    def detect_php_tools_from_config_files(project_root: Path) -> dict[str, bool]:
        ...
        """
        result: dict[str, bool] = {}
        for tool, _, config_files in PHPToolsDetector._TOOLS:
    ...
    def detect_php_tools(project_root: Path) -> dict[str, Any]:
        ...
        """
        composer = PHPToolsDetector.detect_php_tools_from_composer(project_root / "composer.json")
        config = PHPToolsDetector.detect_php_tools_from_config_files(project_root)
        return {
            tool: {"available": composer.get(tool, False) or config.get(tool, False)}
            for tool, _, _ in PHPToolsDetector._TOOLS
        }
    ...
    def main() -> int:
        ...
        result = PHPToolsDetector.detect_php_tools(Path(args.directory))
        print(json.dumps(result, indent=2))
        return 0


if __name__ == "__main__":  # pragma: no cover
    sys.exit(PHPToolsDetector.main())
