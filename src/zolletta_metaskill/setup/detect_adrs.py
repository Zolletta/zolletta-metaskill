...
Usage:
    python3 detect_adrs.py [docs_dir]
...
def _scan_adr_headings(files: list[Path]) -> list[Path]:
    ...
    for f in files:
        ...
        try:
            ...
            continue
        if _ADR_HEADING_RE.search(content):
            confirmed.append(f)
    return confirmed


def _determine_adrs_path(
    adr_files: list[Path], docs_dir: Path
) -> str:
...
def detect_adrs(docs_dir: Path) -> str | None:
    ...
    grep_results = _grep_adr_files(docs_dir)
    if grep_results is not None:
        ...
        # Exclude adr-distilled.md
        candidate_files = [
            f for f in candidate_files if f.name != "adr-distilled.md"
        ]
    ...
    adr_files = _scan_adr_headings(candidate_files)
    if not adr_files:
        return None

    return _determine_adrs_path(adr_files, docs_dir)
...
def main() -> int:
    ...
    adrs_path = detect_adrs(docs_dir)
