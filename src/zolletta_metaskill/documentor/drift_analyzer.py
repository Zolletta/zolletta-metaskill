    ...
    if referenced_source_files:
        ...
        if changed_referenced:
            ...
            issues.append({
                ...
                "category": "factual",
                "description": (
                    f"{count} referenced source file(s) changed since doc was last updated: "
        ...
        if total_code_changes > 20:
            issues.append({
                ...
                "category": "factual",
                "description": (
                    f"{total_code_changes} code files changed in associated directories since doc "
                    f"was last updated (no specific source files referenced in doc — needs review)"
                ),
                "fix_type": "manual",
        ...
        if old_base in {os.path.basename(r) for r in refs["files"]}:
            ...
            if old_still_exists:
                ...
                issues.append({
                    ...
                    "severity": "medium",
                    "category": "referential",
                    "description": f"References '{old_base}' which was renamed to '{new_base}'",
                    "fix_type": "auto",
                    "details": {"old_path": old_name, "new_path": new_name},
                })
  ...
  %(prog)s /path/to/repo --doc-patterns "*.md,*.rst"
    ...
    for doc in doc_files:
        ...
        issues = detect_drift_for_doc(
            repo_path, doc, associated_dirs, renames, current_version,
