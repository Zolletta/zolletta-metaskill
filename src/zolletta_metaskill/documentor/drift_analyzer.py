...
class DriftAnalyzer:
    """Documentation drift analyzer — all helpers as static methods."""
    ...
    DRIFT_CATEGORIES = {"structural", "factual", "referential", "temporal"}
    ...
    def _load_gitignore_patterns(repo_path: str) -> set[str]:
        ...
        `.zolletta-metaskill/`, `.devin/`, `.pytest_cache/` etc.
        """
    ...
    def get_file_last_modified(repo_path: str, file_path: str) -> datetime | None:
        ...
        output = DriftAnalyzer.run_git(repo_path, ["log", "-1", "--format=%aI", "--", file_path])
        if output:
            try:
    ...
    def get_files_changed_since(
        ...
        if scope:
            args += ["--", scope]
        output = DriftAnalyzer.run_git(repo_path, args)
        changes = []
    ...
    def get_current_version_from_git(repo_path: str) -> str | None:
        ...
        output = DriftAnalyzer.run_git(repo_path, ["describe", "--tags", "--abbrev=0"])
        if output:
    ...
    def find_doc_files(repo_path: str, patterns: list[str] | None = None) -> list[str]:
        ...
        skip_dirs |= DriftAnalyzer._load_gitignore_patterns(repo_path)
        ...
        if patterns:
            ...
            extensions = DriftAnalyzer.DOC_EXTENSIONS
    ...
    def find_code_files(repo_path: str, scope: str = "") -> list[str]:
        ...
        for root, dirs, files in os.walk(search_path):
            dirs[:] = [d for d in dirs if d not in skip_dirs]
            for f in files:
                if Path(f).suffix.lower() in DriftAnalyzer.CODE_EXTENSIONS:
                    rel_path = os.path.relpath(os.path.join(root, f), repo_path)
    ...
    def extract_references_from_doc(repo_path: str, doc_path: str) -> dict[str, set[str]]:
        ...
        for match in code_ref_pattern.finditer(content):
            ...
            if Path(candidate).suffix.lower() in DriftAnalyzer.CODE_EXTENSIONS:
    ...
    def detect_drift_for_doc(
        ...
        doc_modified = DriftAnalyzer.get_file_last_modified(repo_path, doc_path)
        if not doc_modified:
            issues.append(
                {
                    "file": doc_path,
        ...
        for ref_file in refs["files"]:
            if Path(ref_file).suffix.lower() not in DriftAnalyzer.CODE_EXTENSIONS:
                continue
            resolved_doc = os.path.normpath(os.path.join(repo_path, doc_dir, ref_file))
            resolved_root = os.path.normpath(os.path.join(repo_path, ref_file))
            if os.path.exists(resolved_doc):
        ...
        if referenced_source_files:
            ...
            for src_file in referenced_source_files:
                changes = DriftAnalyzer.get_files_changed_since(repo_path, since_date, src_file)
                if changes:
                    changed_referenced.append(src_file)

            if changed_referenced:
                count = len(changed_referenced)
                if count > 5:
                    severity = "high"
                elif count > 2:
                    severity = "medium"
                ...
                issues.append(
                    {
                        ...
                        "category": "factual",
                        "description": (
                            f"{count} referenced source file(s) changed "
                            f"since doc was last updated: "
                            f"{', '.join(changed_referenced[:5])}"
                        ),
                        "fix_type": "semi",
        ...
        for old_name, new_name in renames:
            ...
            if old_base in {os.path.basename(r) for r in refs["files"]}:
                # Check if the old name still exists (edge case — always report)
                old_resolved = os.path.normpath(os.path.join(repo_path, doc_dir, old_name))
                old_resolved_root = os.path.normpath(os.path.join(repo_path, old_name))
                old_still_exists = os.path.exists(old_resolved) or os.path.exists(old_resolved_root)

                if old_still_exists:
                    # Edge case: old name exists but as a different file — the reference
                    # might be pointing to the wrong file
                    issues.append(
                        {
                            "file": doc_path,
                            "severity": "high",
                ...
                elif include_referential:
                    # Broad rename detection — only when explicitly requested
                    issues.append(
                        {
                            "file": doc_path,
                            "severity": "medium",
                    ...
                    issues.append(
                        {
                            ...
                            "category": "temporal",
                            "description": (
                                f"References version {doc_version}, current is {current_version}"
                            ),
                            "fix_type": "auto",
                            "details": {
    ...
    def _version_is_older(v1: str, v2: str) -> bool:
        ...
        """
        try:
            parts1 = [DriftAnalyzer._parse_version_part(x) for x in v1.split(".")]
            parts2 = [DriftAnalyzer._parse_version_part(x) for x in v2.split(".")]
            # Pad to same length
    ...
    def generate_report(
        ...
        # Sort by severity
        all_issues.sort(
            key=lambda x: DriftAnalyzer.SEVERITY_ORDER.get(x.get("severity", "info"), 99)
        )
...
Examples:
  ...
  %(prog)s /path/to/repo --doc-patterns "*.md,*.rst"
        ...
        renames = DriftAnalyzer.get_renamed_files(repo_path, "90 days ago")

        # Get current version
        current_version = DriftAnalyzer.get_current_version_from_git(repo_path)
        ...
        filtered = [
            i
            for i in all_issues
            if DriftAnalyzer.SEVERITY_ORDER.get(i.get("severity", "info"), 99) <= min_sev
        ]
