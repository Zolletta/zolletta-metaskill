...
from zolletta_metaskill.adr.distill_report import DistillReport
...
class ADROrchestrator:
    ...
    def refresh(self) -> DistillReport:
        ...
        if report.has_adrs and directive_lines:
            ...
            existing_has_directives = bool(
                ...
                and ADRDistiller.parse_directives(existing_content)
            )
    ...
    def run(argv: list[str] | None = None) -> int:
        ...
        """
        parser = ADROrchestrator.build_parser()
        args = parser.parse_args(argv)

        docs_dir = Path(args.docs_dir)
        if not docs_dir.is_dir():
            msg = ADROrchestrator.missing_docs_error(docs_dir, args.json)
            if args.json:
                print(msg)
            else:
        ...
        print(ADROrchestrator.format_report(report, args.json))
        return 0


def main() -> int:
    """Entry point for the ADR distiller CLI."""
    return ADROrchestrator.run()
