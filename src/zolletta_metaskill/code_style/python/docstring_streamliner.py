...
class DocstringStreamliner:
    ...
    def _is_section_header(line: str) -> bool:
        ...
        return name in DocstringStreamliner.SECTION_HEADERS
    ...
    def parse_docstring(text: str) -> tuple[list[str], list[tuple[str, list[str]]]]:
        ...
        while i < len(lines) and not DocstringStreamliner._is_section_header(lines[i]):
            summary.append(lines[i])
            i += 1

        while i < len(lines):
            if DocstringStreamliner._is_section_header(lines[i]):
                header = lines[i]
                body: list[str] = []
                i += 1
                while i < len(lines) and not DocstringStreamliner._is_section_header(lines[i]):
    ...
    @staticmethod
    def is_trivial_arg_desc(entry_name: str, desc: str, annotation: str | None) -> bool:
        ...
        if not desc:
            return True
        arg_name = DocstringStreamliner._arg_name_from_entry(entry_name)
    ...
    def get_arg_annotations(node: ast.FunctionDef | ast.AsyncFunctionDef) -> dict[str, str | None]:
        ...
        for arg in all_args:
            annotations[arg.arg] = DocstringStreamliner._annotation_str(arg.annotation)
        if args.vararg:
            annotations[args.vararg.arg] = DocstringStreamliner._annotation_str(
                args.vararg.annotation
            )
        if args.kwarg:
            annotations[args.kwarg.arg] = DocstringStreamliner._annotation_str(
    ...
    def get_return_annotation(node: ast.FunctionDef | ast.AsyncFunctionDef) -> str | None:
        ...
        return DocstringStreamliner._annotation_str(node.returns)

    @staticmethod
    def _is_private(name: str) -> bool:
        return (
            name.startswith("_")
            and name not in DocstringStreamliner.DUNDER_METHODS
    ...
    def _detect_prefix_quote(raw: str) -> tuple[str, str] | None:
        ...
        match = DocstringStreamliner._DOCSTRING_RE.match(raw)
        if not match:
            return "", '"""'
    ...
    def _analyze_function(
        ...
        name = node.name
        arg_annotations = DocstringStreamliner.get_arg_annotations(node)
        return_annotation = DocstringStreamliner.get_return_annotation(node)

        # --- Opt-in: strip private / test / nested / obvious-init -------------
        if strip_private and DocstringStreamliner._is_private(name):
            return Finding(path, node.lineno, "private", f"{name}() — private", node, remove=True)
        if (
            strip_tests
            and DocstringStreamliner._is_test_function(name)
            and DocstringStreamliner._is_test_file(path)
        ):
            return Finding(
                path, node.lineno, "test", f"{name}() — test function", node, remove=True
            )
        if strip_nested and DocstringStreamliner._is_nested(node):
            return Finding(
                path,
                node.lineno,
                "nested",
                f"{name}() — nested function",
                node,
                remove=True,
            )
        if (
            strip_obvious_init
            and name == "__init__"
            and DocstringStreamliner._init_is_obvious(arg_annotations)
        ):
            return Finding(
                path,
                node.lineno,
        ...
        summary, sections = DocstringStreamliner.parse_docstring(docstring)
        ...
        for header, body in sections:
            ...
            if header_lower in {"args", "arguments", "parameters"}:
                if DocstringStreamliner._args_section_is_redundant(body, arg_annotations):
                    removed_headers.append("Args")
                    continue
            elif header_lower == "returns" and DocstringStreamliner._returns_section_is_redundant(
        ...
        new_text = DocstringStreamliner.rebuild_docstring(summary, kept_sections)
        remove = not new_text.strip()
        detail = f"removed redundant section(s): {', '.join(removed_headers)}"
        return Finding(
            path, node.lineno, "redundant", detail, node, new_text=new_text, remove=remove
        )
    ...
    def _init_is_obvious(arg_annotations: dict[str, str | None]) -> bool:
        ...
        return all(
            ann is not None for name, ann in arg_annotations.items() if name not in ("self", "cls")
        )

    @staticmethod
    def _args_section_is_redundant(body: list[str], arg_annotations: dict[str, str | None]) -> bool:
        ...
        for name_part, desc in entries:
            arg_name = DocstringStreamliner._arg_name_from_entry(name_part)
            if arg_name in ("self", "cls"):
                continue
            annotation = arg_annotations.get(arg_name)
            if not DocstringStreamliner.is_trivial_arg_desc(name_part, desc, annotation):
                return False
        return True
    ...
    def _returns_section_is_redundant(body: list[str], return_annotation: str | None) -> bool:
        desc = "\n".join(body).strip()
        return DocstringStreamliner.is_trivial_returns_desc(desc, return_annotation)
    ...
    def apply_edits(path: Path, findings: list[Finding]) -> str:
        ...
        for finding in ordered:
            ...
            detected = DocstringStreamliner._detect_prefix_quote(raw_block)
            ...
            if finding.remove or finding.new_text is None:
                ...
                rendered = DocstringStreamliner.render_docstring(
                    indent, prefix, quote, finding.new_text
                )
...
if __name__ == "__main__":  # pragma: no cover
    sys.exit(DocstringStreamliner.main())
