...
class DependencyInversionScanner:
    ...
    def _is_data_class(node: ast.ClassDef) -> bool:
        ...
        for dec in node.decorator_list:
            if (
                ...
                and dec.id in DependencyInversionScanner.DATA_CLASS_MARKERS
            ):
                return True
            if (
                isinstance(dec, ast.Call)
                and isinstance(dec.func, ast.Name)
                and dec.func.id in DependencyInversionScanner.DATA_CLASS_MARKERS
            ):
                return True
        for base in node.bases:
            ...
            if (
                ...
                and base.id in DependencyInversionScanner.DATA_CLASS_MARKERS
            ):
                return True
        return False
        ...
        for node in ast.walk(class_node):
            if isinstance(node, ast.Call):
                ...
                if (
                    ...
                    and func.id in DependencyInversionScanner.DI_CONTAINER_FACTORIES
                ):
                    return True
                if (
                    isinstance(func, ast.Attribute)
                    and func.attr in DependencyInversionScanner.DI_CONTAINER_FACTORIES
                ):
                    return True
        return False
                ...
                if isinstance(node, ast.Assign):
                    for target in node.targets:
                        ...
                        if isinstance(value, ast.Call):
                            created_class = DependencyInversionScanner._get_class_name_from_call(
                                value
                            )
                            if created_class and DependencyInversionScanner._is_real_dependency(
                                created_class
                            ):
                                violations.append(
                                    {
    ...
    def _is_real_dependency(class_name: str) -> bool:
        ...
        if class_name in DependencyInversionScanner.STDLIB_TYPES:
            return False
...
if __name__ == "__main__":  # pragma: no cover
    sys.exit(DependencyInversionScanner.main())
