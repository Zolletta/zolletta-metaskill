...
- An engine registry (`register` / `get` / `get_for_file` / `available_languages`) maps file extensions to engines.
...
- `ModuleInfo` is a lowest-common-denominator abstraction. Language-specific features that do not map to `ModuleInfo` fields are invisible to scanners. Engines that need richer data (e.g., PHP's `parse_raw(path)` for direct tree-sitter access) bypass the protocol.
