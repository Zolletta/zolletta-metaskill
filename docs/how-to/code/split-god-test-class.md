...
Split a test class that tests multiple SUTs (System Under Test) into per-SUT test files. The `src/zolletta_metaskill/patterns/python/test_splitter.py` script automates this — it groups test methods by prefix, creates a new test file per SUT, and writes them to a temp folder. The original file is never modified.
...
- The `src/zolletta_metaskill/patterns/python/test_splitter.py` script from `src/zolletta_metaskill/patterns/python/`
...
python3 src/zolletta_metaskill/patterns/python/test_splitter.py <test_file>
