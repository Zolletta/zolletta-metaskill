...
The review flags source files that exceed the project's configured maximum line count (`max_file_length`, default 300). The check is enforced by the language-agnostic `file_length_scanner.py` — it counts lines, so it works for any language. Files that legitimately exceed the limit (generated code, large enums) can be exempted via the scanner's `--exclude` flag or by raising the threshold in `settings.json`.
